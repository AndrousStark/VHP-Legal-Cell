/** VHP Legal Cell — 12 Khetras (Regions) mapping to Indian states */

export interface KhetraConfig {
  id: string;
  nameEn: string;
  nameHi: string;
  color: string;
...
/**
 * 12 VHP Khetras mapped to Indian states.
 * Colors chosen for visual distinction on the map.
 */
export const KHETRAS: KhetraConfig[] = [
  {
    id: "uttaranchal",
    nameEn: "Uttaranchal Khetra",
...
/** Find which Khetra a state belongs to */
export function getKhetraForState(stateName: string): KhetraConfig | undefined {
  return KHETRAS.find((k) =>
    k.states.some((s) => s.toLowerCase() === stateName.toLowerCase())
  );
}

/** Get color for a state based on its Khetra */
export function getStateColor(stateName: string): string {
  return getKhetraForState(stateName)?.color ?? "#9E9E9E";
}
...
export const MOCK_REGION_STATS: Record<string, {
  ...
  "braj-khetra": { cases: 85, members: 120, activities: 12, resolutionRate: 78 },
